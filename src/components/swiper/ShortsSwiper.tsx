      ...
      >
        {
          shortVideos.map((short) => (
            <SwiperSlide key={short.id} className="relative lg:pr-15">
              {/* left section */}
              ...
              < div className="flex w-max h-full mx-auto" >
                <div className="relative h-full bg-amber-400 rounded-xl overflow-hidden">
                  ...
                  {/* left section mobile */}
                  <div className="flex 2xl:hidden absolute z-3 left-2 lg:left-4 bottom-2 lg:bottom-4 flex-col gap-2 lg:gap-3 max-w-full pr-4">
                    <div className="flex items-center gap-2 h-10">
                      <div className="size-8 lg:size-9 rounded-full overflow-hidden">
                        <Image src={short.avatar_image} width={40} height={40} alt="avatar" className="w-full h-full object-cover" />
                      </div>
                      <span className="text-sm lg:text-base">@{short.owner}</span>
                      <Button className="rounded-full bg-secondColor text-mainColor py-1.5 lg:py-2 h-max! lg:text-base">Subscribe</Button>
                    </div>
                    <div className="lg:text-[20px] font-bold w-full line-clamp-1">
                      <span>{short.title}</span>
                      {short.hashtags.map((item, i) => (
                        <span key={i} className="mr-1">
                          {" "}
                ...
                < div className="flex flex-col gap-2 lg:gap-4 justify-end items-center px-2 lg:px-3 text-xs lg:text-sm font-medium" >
                  {
                    shortButtons(short).map((item, i) => (
                      <div key={item.label + `${i}`} className="flex flex-col gap-1 text-center">
                        <Button variant="iconVariant" className="bg-buttonBgColor lg:p-3! [&_svg:not([class*='size-'])]:size-5">
                          {item.icon}
                        </Button>
                        <p>{item.label}</p>
                      </div>
                  ...
                  <div className="size-8 lg:size-10 rounded-sm overflow-hidden">
                    <Image src={short.avatar_image} width={40} height={40} alt="avatar" className="w-full h-full object-cover" />
                  </div>
                </div >
              </div >
