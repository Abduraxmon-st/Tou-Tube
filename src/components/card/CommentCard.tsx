...
export const CommentCard = ({ comment }: { comment: Comment }) => {
  ...
  return (
    <div className="relative pr-5 lg:pr-10">
      <VideosSelect videoType="comments" buttonClass="absolute -right-4 lg:right-0 top-0" align="end"/>
      <div className="flex gap-4 relative">
        ...
        <div className="flex-1 flex flex-col gap-1">
            ...
            answersOpen && (
              comment.answers.map((answer, i) => (
                <div key={i} className="flex gap-4">
                  <div className="relative">
                    <div className={`absolute top-0 -left-8.75 size-5 border-b-3 rotate-56 border-thirdColor/20 rounded-full`} />
          ...
          {comment.answers.length > 0 && (
            <div>
              <div onClick={() => setAnswersOpen(!answersOpen)} className="text-sm mt-4 cursor-pointer w-max">
                {comment.answers.length} answers <ChevronDown className={`inline ${answersOpen && "rotate-180"}`} />
              </div>
            </div>
          )}
        </div>
