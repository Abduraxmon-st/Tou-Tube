...
export const ListCard = ({ videoType = "classic", ownerVisible = true, video }: ListCardProps) => {
    return (
        <div className=" gap-x-3 gap-y-7.5 xm:gap-y-5 w-full flex flex-col">
            <Card className="relative bg-transparent border-0 py-0! gap-0 shadow-none rounded-none text-secondColor cursor-pointer">
                <div className="relative flex gap-2">
                    ...
                    <div className="flex-1 relative pr-6 p-3 sm:p-2 3md:p-4">
                        ...
                        <div className="flex items-center gap-2 py-3">
                            {ownerVisible && <Avatar icon={video.avatar_image} className="size-6" />}
                            <span className="text-thirdColor text-xs font-medium">{video.owner}</span>
                        </div>
                        <span className="text-thirdColor text-sm line-clamp-2">
                            {video.description}
                        </span>
                    </div>
                </div>
